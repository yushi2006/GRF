...
class CrossModalAttentionEncoder(nn.Module):
    def __init__(self, d_model, nhead, d_ffn, num_layers, dropout=0.1):
        ...
        decoder_layer_1 = nn.TransformerDecoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=d_ffn, dropout=dropout, batch_first=True, activation="gelu")
        self.decoder_1 = nn.TransformerDecoder(decoder_layer_1, num_layers=num_layers)
        decoder_layer_2 = nn.TransformerDecoderLayer(d_model=d_model, nhead=nhead, dim_feedforward=d_ffn, dropout=dropout, batch_first=True, activation="gelu")
    ...
    def forward(self, x_state, l_state, x_modality, l_modality):
        x_state_pe = self.pos_encoder(x_state); x_modality_pe = self.pos_encoder(x_modality)
        state_mask = self._create_padding_mask(l_state, x_state.size(1)); modality_mask = self._create_padding_mask(l_modality, x_modality.size(1))
        enriched_state = self.decoder_1(tgt=x_state_pe, memory=x_modality_pe, tgt_key_padding_mask=state_mask, memory_key_padding_mask=modality_mask)
        enriched_modality = self.decoder_2(tgt=x_modality_pe, memory=x_state_pe, tgt_key_padding_mask=modality_mask, memory_key_padding_mask=state_mask)
        pooled_state = self._pool(enriched_state, l_state); pooled_modality = self._pool(enriched_modality, l_modality)
...
class SentimentRegressionHead(nn.Module):
    ...
    def forward(self, x): return self.regressor(x)