...
class MultimodalDataset(Dataset):
    def __init__(self, modalities: List[ModalityData], labels: torch.Tensor):
        self.modalities = modalities; self.labels = labels
    def __len__(self): return len(self.labels)
    def __getitem__(self, idx):
        return tuple(chain(*[(mod.features[idx], mod.lengths[idx]) for mod in self.modalities])) + (self.labels[idx],)
...
def load_mosi_data_regression(data_path: str, num_classes: int):
    ...
    with open(data_path, "rb") as f: data = pickle.load(f, encoding="latin1")
    processed_data = {}
    for split in ["train", "valid", "test"]:
        ...
        modalities = { "text": torch.tensor(data[split]["text"], dtype=torch.float32), "audio": torch.tensor(data[split]["audio"], dtype=torch.float32), "vision": torch.tensor(data[split]["vision"], dtype=torch.float32), }
        processed_data[split] = {"labels": labels, "modalities": {name: ModalityData(features, torch.full((len(features),), features.shape[1]), name) for name, features in modalities.items()}}
...
def create_dataloaders(all_data: dict, order_keys: list, batch_size: int):
    ...
    train_dataset = MultimodalDataset(initial_modalities_train, all_data['train']['labels'])
    valid_dataset = MultimodalDataset([all_data['valid']['modalities'][k] for k in order_keys], all_data['valid']['labels'])
    test_dataset = MultimodalDataset([all_data['test']['modalities'][k] for k in order_keys], all_data['test']['labels'])
