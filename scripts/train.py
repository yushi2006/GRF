...
from src import MULTModel, SentimentClassifierHead


def train_loop(
    dataloader, optimizer, scheduler, loss_fn, encoder, classifier, device, epochs
):
    encoder.to(device)
    ...
    for epoch in range(epochs):
        ...
        classifier.train()
        for batch in tqdm(
            dataloader, desc=f"Epoch {epoch + 1}/{epochs} [Training]", leave=False
        ):
            ...
            torch.nn.utils.clip_grad_norm_(
                chain(encoder.parameters(), classifier.parameters()), max_norm=0.8
            )
            optimizer.step()
        scheduler.step()
...
def run_training(
    hparams: Dict, loaders: Dict, device: torch.device, save_dir: str
) -> str:
    print("\n" + "=" * 20 + " STARTING TRAINING PHASE " + "=" * 20)
    train_start_time = time.time()

    encoder = MULTModel(
        d_in_t=loaders["d_in_t"],
        d_in_a=loaders["d_in_a"],
        d_in_v=loaders["d_in_v"],
        **hparams["model"],
    ).to(device)
    classifier = SentimentClassifierHead(
        hparams["model"]["d_model"], hparams["num_classes"]
    ).to(device)

    class_counts = torch.bincount(loaders["train"].dataset.labels)
    class_weights = len(loaders["train"].dataset.labels) / (
    ...
    with mlflow.start_run(run_name="MOSI_MULT_Baseline_Training"):
        ...
        for epoch in range(hparams["epochs"]):
            ...
            for batch in tqdm(
                loaders["train"],
                desc=f"Epoch {epoch + 1}/{hparams['epochs']}",
                leave=False,
            ):
            ...
            val_metrics = evaluate(
                loaders["valid"], encoder, classifier, loss_fn, device
            )
            print(
                f"Epoch {epoch + 1} | Val F1: {val_metrics['f1']:.4f} | Val Acc: {val_metrics['accuracy']:.4f}"
            )
            mlflow.log_metrics(
    ...
    print(
        f"--- Training finished in {total_train_duration / 60:.2f} minutes. Best model saved to {best_model_path} ---"
    )
